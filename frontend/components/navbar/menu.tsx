            ...
            {open && (
              <>
                <motion.div
                  ...
                  className="fixed w-[calc(100%-2rem)] h-[calc(100%-2rem)] items-center justify-center z-[2000] inset-0 bg-[url(/bgbtn.png)] bg-contain border-2 m-4 flex flex-col gap-6"
                >
                  <Button
                    className="p-1 absolute h-max w-max m-4 top-0 right-0 shadow"
                    onClick={() => setOpen(false)}
                  >
                    <Plus className="rotate-45 icon-shadow" />
                  </Button>

                  <ul className="flex flex-col gap-4 justify-center items-center text-2xl">
                    {links.map((link, i) => (
                      <li key={i}>
                  ...
                  <ul className="flex items-center justify-end gap-2">
                    ...
                    <li>
                      <LinkButton
                        href="/login"
                        className="py-1 px-2 text-base shadow"
                      >
                        Login
                      </LinkButton>
                    </li>
                ...
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  onClick={() => setOpen(false)}
                  className="fixed w-full h-full z-[1500] bg-zinc-800/20 inset-0"
                ></motion.div>
              </>
            )}
          </AnimatePresence>,
